...
pub fn PageDrawer() -> impl IntoView {
    ...
    view! {
        <Article>
            ...
            <Box attr:style="display: flex; flex-direction: row; justify-content: flex-start; align-items: flex-start; border: 4px solid gray; width: 100%; height: 20em; overflow: hidden;">
                ...
                <Box attr:style="padding: 0.5em; display: flex; flex-direction: column; overflow-y: scroll; width: 100%; height: 100%;">
                    <p>"Scroll ↓"</p>
                    <Stack spacing=Size::Em(0.5)>
                        {(0..8).map(|_| view! { <Skeleton height=Size::Em(3.0)/> }).collect_view()}
                    </Stack>
            ...
            <Box attr:style="position: relative; display: flex; flex-direction: row; justify-content: flex-start; align-items: flex-start; border: 4px solid gray; width: 100%; height: 20em; overflow: hidden;">
                <Box attr:style="padding: 0.5em; display: flex; flex-direction: column; overflow-y: scroll; width: 100%; height: 100%;">
                    <p>"Scroll ↓"</p>
                    <Stack spacing=Size::Em(0.5)>
                        {(0..8).map(|_| view! { <Skeleton height=Size::Em(3.0)/> }).collect_view()}
